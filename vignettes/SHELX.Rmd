...
names(obj_shelxc)
```
Plot $Delta*F/sig(Delta*F)$ vs resolution using the function`plot_SHELX`.

```{r}
plot_SHELX(obj_shelxc, var = obj_shelxc$I_sig, type = "shelxc")
```
...
Plot inverted and orginal hand.

```{r}
plot_SHELX(filename = obj_shelxe_i, filename_e = obj_shelxe_o, type = "shelxe")
```
